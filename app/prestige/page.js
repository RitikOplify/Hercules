...
function page() {
  const prestigeServiceRef = useRef([]);
  useGsap(prestigeServiceRef, {
    stagger: true,
  });
  return (
    <>
      <div className="relative min-h-[75vh] md:min-h-[90vh] bg-[url(../public/Prestige/prestige_mob_banner.png)]  md:bg-[url(../public/Prestige/banner_prestige.png)] bg-cover bg-center  flex pt-20 md:pt-0 items-end justify-start overflow-hidden">
        {/* <Image
        ...
        <div className="w-full md:w-1/2 flex md:hidden flex-col justify-center items-center md:items-start ps-0 md:ps-20">
          <h2
            ...
            ref={(el) => (prestigeServiceRef.current[2] = el)}
          >
            Defining Excellence Through Timepieces Crafted for the Distinguished
            Few.
          </h2>
          <Button
            ref={(el) => (prestigeServiceRef.current[3] = el)}
            color={"white"}
            text={"DISCOVER"}
            className={"mt-8 bg-[#9b8959]"}
            // url={"#contact-us-form"}
          />
          <Image
            ref={(el) => (prestigeServiceRef.current[4] = el)}
            src="/Prestige/prestige_sec_watch.png"
            alt="Luxury Watch"
            width={491}
            height={623}
            priority
